...
function Login() {
  ...
  const changeHandler = (e) => (setInputpending(false), setInput(true), setBody({ ...body, [e.target.name]: e.target.value }));

  const postLogin = () => {
    if (!body.email || !body.password) return setInput(false), setInputpending(false), toast.error('Data cannot be empty');
    axios
