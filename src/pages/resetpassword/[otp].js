...
function Reset() {
  ...
  const resetPassword = () => {
    ...
    axios
      ...
      .catch((err) => (setInput(false), setInputpending(false), toast.error(err.response.data.msg)));
  };
  ...
  return (
    <>
        ...
        <div className={css.content_right}>
          <div className={css.content_form}>
            ...
            <div className={css.password}>
              ...
              <input type={type} name="newPassword" id="" onChange={valuePassword} placeholder="Create new password" />
              <i className={icon} onClick={handleToggle1}></i>
            </div>
            <div className={css.password}>
              <i className={`fa-solid fa-lock ${inputpending ? 'text-secondary' : input ? 'text-primary' : 'text-danger'}`}></i>
              <input type={type_} name="confirmPassword" id="" onChange={valueConfirm} placeholder="Confirm new password" />
      ...
      <ToastContainer position="top-center" autoClose={3000} hideProgressBar={false} closeOnClick={true} pauseOnHover={true} draggable={true} theme="light" />
    </>
  );
}
